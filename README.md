...
一个面向小米 / HyperOS 设备的 LSPosed 风扇增强模块，用于在系统散热风扇功能中新增“狂暴模式”，并通过 PowerKeeper 接入官方风扇控制链路。
...
Xiaomi_FanUltra 的核心作用是在系统原有散热风扇功能中新增一个额外模式：
...
在官方智能调频、静谧模式、高速强冷之外，新增“狂暴模式”。用户可以像选择官方档位一样，在系统设置或控制中心中直接选择。
...
用于避免切回智能调频后底层 `target_level` 仍残留 `4`。
...
6. 当前稳定版本不包含自定义 `pwm_duty` 滑块功能。此前验证表明 PowerKeeper/IMiCharge 对 `pwm_duty` 写入通路不可靠，因此未纳入稳定主线。
