...
`Xiaomi_FanUltra` 是一个面向小米 / HyperOS 设备的轻量级 LSPosed 风扇增强模块。

模块会在系统原有散热风扇功能中新增 **“狂暴模式”**，并通过 Hook Settings、SystemUI 与 PowerKeeper，将新增模式接入官方风扇控制链路，实现比官方“高速强冷”更高的风扇档位。
...
- 本项目基于 MIT License 开源免费，欢迎学习、交流、适配与改进。
...
模块不绕过系统直接粗暴写 sysfs 节点，而是接入官方调用链：
...
整体行为更接近官方风扇档位，也便于跟随官方场景策略。

### 遵循官方使用范围和场景限制

狂暴模式会参考官方散热风扇设置中的相关状态，例如：
...
从狂暴模式切回智能调频时，模块会在 PowerKeeper 侧执行中转桥接流程：
...
| Hook 方法 | `J(String,String)` / `handleMessage(Message)` / `o()` |
